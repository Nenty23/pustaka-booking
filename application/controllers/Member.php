...
class Member extends CI_Controller { 
    ...
    private function _login() 
    {
            ...
            if ($user['is_active'] == 1) {
                ...
                if (password_verify($password, $user['password'])) { 
                    $data = [ 'email' => $user['email'],
                            ...
                            'id_user' => $user['id'], 
                            'nama' => $user['nama'] ]; 
                        $this->session->set_userdata($data); redirect('Member/myProfil'); 
                    } else { 
        ...
        { 
            ...
            $this->ModelUser->simpanData($data); $this->session->set_flashdata('pesan', '<div class="alert alert-success alert-message" role="alert">Selamat!! akun anggota anda sudah dibuat.</div>'); redirect(base_url()); 
        }
